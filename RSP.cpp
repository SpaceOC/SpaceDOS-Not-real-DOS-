...
void RSP() {
    ...
    LogMessage(true, "| [RSP] - Игрок выбрал число: " + to_string(a), "", 000);

    int v = rand() % 3 + 1;

    LogMessage(true, "| [RSP] - Система выбрала число: " + to_string(v), "", 000);

    if (a == v) {
        cout << "Tie!" << endl;
        LogMessage(true, "| [RSP] - Случилась ничья", "", 000);
    }
    else if (a == 1 && v == 2) {
        cout << "The rock breaks the scissors! The computer lost." << endl;
        cout << "----------------------------------------------------------" << endl;
        LogMessage(true, "| [RSP] - Компьютер проиграл / Игрок выйграл", "", 000);
    }
    else if (a == 2 && v == 3) {
        ...
        cout << "----------------------------------------------------------" << endl;
        LogMessage(true, "| [RSP] - Компьютер выйграл / Игрок проиграл", "", 000);
    }
    else if (a == 3 && v == 2) {
        ...
        cout << "----------------------------------------------------------" << endl;
        LogMessage(true, "| [RSP] - Игрок выбрал неверное число", "", 000);
    }
}